#!/usr/bin/env python3
"""
CSV-Based Content Analysis Tool for Research Papers
...
class CSVContentAnalyzer:
    def __init__(self, api_keys=None):
        """
        Initialize the CSVContentAnalyzer class.
...
"""
    ...
    def load_papers_from_csv(self, csv_path):
        ...
        try:
            ...
            if 'abstract' in available_cols:
                ...
                print(f"📝 Papers with abstracts: {valid_abstracts}/{len(df)} ({valid_abstracts/len(df)*100:.1f}%)")
    ...
    def save_analysis_result(self, paper_info, analysis_result, output_dir, index):
        ...
        try:
            ...
            print(f"💾 Saved analysis to {os.path.basename(output_path)}")
            return output_path
    ...
    def analyze_papers_from_csv(self, csv_path, output_dir=None, max_papers=None, start_index=0):
        ...
        """
        print("🧠 Starting CSV Content Analysis")
        print("=" * 50)
        ...
        print(f"🎯 Analyzing papers {start_index + 1} to {end_index} (total: {len(papers_to_analyze)})")
        
        # Check if API keys are available
        if not self.api_keys:
            print("⚠️ No API keys available. Cannot perform analysis.")
            print("💡 Use analyzer.set_api_keys(['your_key1', 'your_key2']) to add API keys")
            return []
        ...
        for idx, (_, row) in enumerate(papers_to_analyze.iterrows(), start=start_index):
            try:
                ...
                print(f"\n📄 Processing paper {idx + 1}/{end_index}: {title[:100]}...")
                ...
                # Show what we have for analysis
                info_summary = []
                if paper_info.get('title'):
                    info_summary.append("title")
                if paper_info.get('abstract'):
                    info_summary.append("abstract")
                if paper_info.get('authors'):
                    info_summary.append("authors")
                print(f"📋 Available info: {', '.join(info_summary)}")
        ...
        if results:
            summary_path = self.create_analysis_summary(results, output_dir)
            if summary_path:
    ...
    def create_analysis_summary(self, analysis_files, output_dir):
        ...
        print(f"📋 Creating summary from {len(analysis_files)} analysis files...")
        summary_data = []
        ...
        if summary_data:
            ...
            print(f"📋 Summary saved with {len(summary_data)} entries")
            return summary_path
        else:
...
if __name__ == "__main__":
    print("🧪 Testing CSV Content Analyzer...")
    ...
    if os.path.exists(consolidated_file):
        ...
        print(f"📄 Testing with {consolidated_file}")
        
        # Test loading and preparation (without API keys)
